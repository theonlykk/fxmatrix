...
//+------------------------------------------------------------------+
void V2_SyncAllCaps(const string cap_namespace, const bool is_long, const int layer_count)
{
...
//+------------------------------------------------------------------+
void V2_EvalBothCaps(const string cap_namespace,
                     const bool is_long,
                     ...
                     string &eval_log)
{
   gbp_blocked = V2_GbpCapBlocksNewAdd(cap_namespace, is_long, gbp_threshold);
   eur_blocked = V2_EurCapBlocksNewAdd(cap_namespace, is_long, eur_threshold);
   eval_log = StringFormat(
      "GBP cap: blocked=%s net=%.0f threshold=%d | EUR cap: blocked=%s net=%.0f threshold=%d",
      gbp_blocked ? "true" : "false",
      V2_GbpNetExposure(cap_namespace),
      gbp_threshold,
      eur_blocked ? "true" : "false",
      V2_EurNetExposure(cap_namespace),
      eur_threshold);
}

//+------------------------------------------------------------------+
bool V2_AnyCapBlocksNewAdd(const string cap_namespace,
                           const bool is_long,
                           const int gbp_threshold,
                           const int eur_threshold)
{
   ...
   string eval_log = "";
   V2_EvalBothCaps(cap_namespace, is_long, gbp_threshold, eur_threshold,
                   gbp_blocked, eur_blocked, eval_log);
