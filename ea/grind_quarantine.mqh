                         ...
                         const ulong now_ms)
{
   if(invariant_ok) {
      if(g_grind_quarantined) {
         ...
         Print(Grind_LogTag(), "INFO GRIND_QUARANTINE release reason=", g_grind_quarantine_reason,
   ...
   if(!g_grind_quarantined) {
      ...
      Print(Grind_LogTag(), "WARN GRIND_QUARANTINE enter reason=", reason);
   ...
   if(elapsed >= GRIND_QUARANTINE_MIN_MS &&
      ...
      Grind_ArchiveMarker("CRITICAL", "QUARANTINE_HALT", reason, 0,
                          StringFormat("{\"ms\":%I64u,\"checks\":%d}",
                                       elapsed, g_grind_quarantine_checks));
      Print(Grind_LogTag(), "CRITICAL GRIND_QUARANTINE halt reason=", reason,
            " ms=", elapsed, " checks=", g_grind_quarantine_checks);
      return GRIND_INV_HALT;
   }
