//+------------------------------------------------------------------+
//| fxmatrix_v2_eurusd.mq5 — V2 dual instance EURUSD (MM_LONG_EURUSD + MM_SHORT_EURUSD)
...
#define V2_EA_NAME              "fxmatrix_v2_eurusd_instrumented"
...
#include "fxmatrix_v2_instrumentation.mqh"
...
bool Long_Adr013ClampBuy(const double theoretical, double &out_price) {
   ...
   const bool clamped = V2_Adr013ClampCoreBuy(theoretical, bid, point, stops, digits, out_price);
   if(clamped)
      V2_InstRecordClampEvent(true);
   return clamped;
}
...
bool Long_ReplacePendingBuy(ulong &ticket_ref, const double price, const ulong magic, const string comment) {
   if(V2_L0RestingWithinDeadband(ticket_ref, price, InpQuoteSpread, InpL0DeadbandMult, EurUsd_L0DeadbandSpreadRef())) {
      ...
      V2_InstRecordDeadbandSkip(true);
      return false;
   }
...
void Long_OnNewBar() {
   ...
   double bid_lvl;
   bool long_l0_clamped = Long_Adr013ClampBuy(bid_theoretical, bid_lvl);
   ...
   if (n == 0) {
      ...
      if(Long_ReplacePendingBuy(g_long_l0_ticket, bid_lvl, MM_LONG_V2, "V2_L0")) {
         V2_InstOnL0QuotePlaced(true, long_l0_clamped);
         if(InpVerboseLog)
...
void Long_HandleDealFill(const ulong deal_ticket, const ulong position_ref) {
   ...
   if (entry_type == DEAL_ENTRY_IN &&
      ...
      const bool long_l0_fill = (order_ticket == g_long_l0_ticket) ||
                             (ArraySize(g_long_layers) == 0 && !is_reload);
...
int Long_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_eurusd_long init magic=", MM_LONG_V2,
...
void Long_OnDeinit(const int reason) {
   ...
   V2_InstPrintSideStats("V2_INST_LONG |", g_v2_inst_long);
}
...
bool Short_ComputeOfferSignal(double &offer_theoretical) {
   ...
   double dynamic_hs = V2_L0DynamicHalfSpread(
      ...
      effective_multiplier,
      live_spread_price,
      Short_PipsToPrice(InpPassivityBuffer));
...
bool Short_Adr013ClampSell(const double theoretical, double &out_price) {
   ...
   const int stops = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   const int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   const bool clamped = V2_Adr013ClampCoreSell(theoretical, ask, point, stops, digits, out_price);
   if(clamped)
      V2_InstRecordClampEvent(false);
   return clamped;
}
...
bool Short_ReplacePendingSell(ulong &ticket_ref, const double price, const ulong magic, const string comment) {
   if(V2_L0RestingWithinDeadband(ticket_ref, price, InpQuoteSpread, InpL0DeadbandMult, EurUsd_L0DeadbandSpreadRef())) {
      ...
      V2_InstRecordDeadbandSkip(false);
      return false;
   }
...
void Short_OnNewBar() {
   ...
   double offer_lvl;
   bool short_l0_clamped = Short_Adr013ClampSell(offer_theoretical, offer_lvl);
   ...
   if (n == 0) {
      ...
      if(Short_ReplacePendingSell(g_short_l0_ticket, offer_lvl, MM_SHORT_V2, "V2_L0")) {
         V2_InstOnL0QuotePlaced(false, short_l0_clamped);
         if(InpVerboseLog)
...
void Short_HandleDealFill(const ulong deal_ticket, const ulong position_ref) {
   ...
   if (entry_type == DEAL_ENTRY_IN &&
      ...
      const bool short_l0_fill = (order_ticket == g_short_l0_ticket) ||
                              (ArraySize(g_short_layers) == 0 && !is_reload);
...
int Short_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_eurusd_short init magic=", MM_SHORT_V2,
         " WIDEN=", InpWidenRatio, " reload_flat=1");
   return INIT_SUCCEEDED;
}
...
void Short_OnDeinit(const int reason) {
   ...
   V2_InstPrintSideStats("V2_INST_SHORT |", g_v2_inst_short);
}
...
int OnInit() {
   if(InpEaseDepthFull <= InpEaseDepthStart || InpEaseDepthStart < 0 || InpEaseDepthFull < 0) {
      Print("ERROR: fxmatrix_v2_eurusd invalid ease depth inputs — InpEaseDepthStart=",
   ...
   Print("INFO: fxmatrix_v2_eurusd init MM_LONG_V2=", MM_LONG_V2,
   ...
   if(long_orphan && short_orphan) {
      Print("ERROR: fxmatrix_v2_eurusd OnInit FAILED — both instances have orphaned positions. ",
            "EA will not attach. Resolve all orphans manually before reattaching.");
      return INIT_FAILED;
   }

   if(long_orphan || short_orphan) {
      Print("WARNING: fxmatrix_v2_eurusd partial startup — halted instance(s) will not trade; ",
