//+------------------------------------------------------------------+
//| fxmatrix_v2_dual.mq5 — Stage-3 dual-topology (LONG + SHORT)      |
//| Generated by temp/build_v2_dual_ea.py — do not hand-edit.         |
//| Does NOT modify fxmatrix_v2_long.mq5 / fxmatrix_v2_short.mq5      |
//+------------------------------------------------------------------+
...
bool Long_SetExitTakeProfit(const int layer_idx) {
   ...
   double tp = g_long_layers[layer_idx].exit_target;
   MqlTradeRequest req = {};
   MqlTradeResult  res = {};
...
void Long_EnsureAddNext() {
   ...
   if (add_price <= 0.0)
      return;
   g_long_add_ticket = Long_PlaceBuyLimit(add_price, 20261000, g_long_last_exit_valid ? "V2_Reload" : "V2_Add");
}
...
void Long_OnNewBar() {
   ...
   if (n == 0) {
      ...
      Long_ReplacePendingBuy(g_long_l0_ticket, bid_lvl, 20261000, "V2_L0");
      if (InpVerboseLog)
...
int Long_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_long init magic=20261000/20261002",
...
bool Short_SetExitTakeProfit(const int layer_idx) {
   ...
   double tp = g_short_layers[layer_idx].exit_target;
   MqlTradeRequest req = {};
   MqlTradeResult  res = {};
   req.action   = TRADE_ACTION_SLTP;
   req.symbol   = _Symbol;
...
void Short_EnsureAddNext() {
   ...
   if (add_price <= 0.0)
      return;
   g_short_add_ticket = Short_PlaceSellLimit(add_price, 20261100, g_short_last_exit_valid ? "V2_Reload" : "V2_Add");
}
...
void Short_OnNewBar() {
   ...
   if (n == 0) {
      ...
      Short_ReplacePendingSell(g_short_l0_ticket, offer_lvl, 20261100, "V2_L0");
      if (InpVerboseLog)
...
int Short_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_short init magic=20261100/20261102",
         " WIDEN=", InpWidenRatio, " reload_flat=1");
   return INIT_SUCCEEDED;
}
...
int OnInit() {
   Print("INFO: fxmatrix_v2_dual init LONG=20261000/20261002 SHORT=20261100/20261102");
   Long_OnInit();
   Short_OnInit();
   return INIT_SUCCEEDED;
}
