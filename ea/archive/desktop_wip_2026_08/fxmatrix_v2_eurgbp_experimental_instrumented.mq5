//+------------------------------------------------------------------+
//| fxmatrix_v2_eurgbp_experimental.mq5 — TEMP EXPERIMENTAL ONLY      |
//| Native return-based EURGBP sigma (replaces MathMax leg sigma).    |
//| Do NOT deploy — verification sweep artifact in temp/.             |
//+------------------------------------------------------------------+
...
#define V2_EA_NAME              "fxmatrix_v2_eurgbp_experimental_instrumented"
...
#include "fxmatrix_v2_signal_experimental.mqh"
...
void Long_EnsureAddNext() {
   ...
   if (add_price <= 0.0)
      return;
   if(!g_long_last_exit_valid &&
      V2_AnyCapBlocksNewAdd(true, InpGbpCapThreshold, InpEurCapThreshold))
      return;
...
bool Long_ComputeBidSignal(double &bid_theoretical) {
   ...
   double eg_closes[];
   if(!V2_CopyM5Closes(_Symbol, eg_closes))
      return false;
   double sig_native = 0.0;
   if(!V2_ReturnSigmaFromCloses(eg_closes, sig_native))
      return false;
...
void Short_EnsureAddNext() {
   ...
   if (add_price <= 0.0)
      return;
   if(!g_short_last_exit_valid &&
      V2_AnyCapBlocksNewAdd(false, InpGbpCapThreshold, InpEurCapThreshold))
      return;
...
void Short_AppendLayer(const double entry_price, const ulong entry_ticket,
   ...
   V2_SyncAllCaps(false, ArraySize(g_short_layers));
