//+------------------------------------------------------------------+
//| fxmatrix_v2_long.mq5 — Stage-1 V2 validation (MM_LONG only)      |
//| ADR-092 minimal: BC signal, ADR-013, reload_flat, WIDEN_RATIO     |
//| Does NOT modify FXMatrix.mq5                                      |
//+------------------------------------------------------------------+
...
input ulong  InpMagicEntry        = 20261000;
input ulong  InpMagicExit         = 20261002;
...
void ReplacePendingBuy(ulong &ticket_ref, const double price, const ulong magic, const string comment) {
   CancelTicket(ticket_ref);
   ticket_ref = PlaceBuyLimit(price, magic, comment);
}
...
void EnsureAddNext() {
   ...
   if (add_price <= 0.0)
      return;
   g_add_ticket = PlaceBuyLimit(add_price, InpMagicEntry, g_last_exit_valid ? "V2_Reload" : "V2_Add");
}
...
void OnNewBar() {
   ...
   if (!ComputeBidSignal(bid_theoretical))
      return;

   double bid_lvl;
   Adr013ClampBuy(bid_theoretical, bid_lvl);

   int n = ArraySize(g_layers);
   if (n == 0) {
      g_last_exit_valid = false;
      ReplacePendingBuy(g_l0_ticket, bid_lvl, InpMagicEntry, "V2_L0");
      if (InpVerboseLog)
         Print("DIAG V2 | event=l0_quote | bid_theo=", DoubleToString(bid_theoretical, 5),
...
int OnInit() {
   ...
   Print("INFO: fxmatrix_v2_long init magic=", InpMagicEntry, "/", InpMagicExit,
