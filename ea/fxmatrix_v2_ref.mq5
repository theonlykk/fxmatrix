//+------------------------------------------------------------------+
//| fxmatrix_v2_ref.mq5 — Phase 1 refactor (GBPUSD preset, parity target: fxmatrix_v2.mq5)
...
#include "fxmatrix_v2_cross_exposure_cap.mqh"
...
V2CrossExposureCapConfig g_v2_cross_cap;
struct LongV2Layer {
   double entry_price;
   double exit_target;
   ulong  entry_ticket;
...
void Long_EnsureAddNext() {
   ...
   if(!g_long_last_exit_valid &&
      ...
      V2_CrossCapRecordBlock();
      if(InpVerboseLog)
         Print("INFO V2_LONG | GBP cap blocked new add net=", V2_CrossCapNetExposure(g_v2_cross_cap),
...
int Long_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_long init magic=MM_LONG_V2",
...
void Short_EnsureAddNext() {
   ...
   if(!g_short_last_exit_valid &&
      ...
      V2_CrossCapRecordBlock();
      if(InpVerboseLog)
         Print("INFO V2_SHORT | GBP cap blocked new add net=", V2_CrossCapNetExposure(g_v2_cross_cap),
               " threshold=", InpGbpCapThreshold);
      return;
   }
...
int Short_OnInit() {
   ...
   Print("INFO: fxmatrix_v2_short init magic=MM_SHORT_V2",
         " WIDEN=", InpWidenRatio, " reload_flat=1");
   return INIT_SUCCEEDED;
}
...
int OnInit() {
   ...
   Print("INFO: fxmatrix_v2_ref init MM_LONG_V2=", MM_LONG_V2,
