...
{
   string built = GrindCommentBuild("OPT", "L", 0, "ENT");
   string slot, side, role;
   int layer;
   AssertTrue("T2 parse", GrindCommentParse(built, slot, side, layer, role));
   AssertEqStr("T2 slot", slot, "OPT");
   AssertEqStr("T2 side", side, "L");
   AssertTrue("T2 layer", layer == 0);
   AssertEqStr("T2 role", role, "ENT");
}
