...
for window_name, path in WINDOWS.items():
    ...
    for spacing_mode in ["reload_anchor", "reload_flat"]:
        for mode_name, mode in [("MM_LONG", simv7.BiasMode.LONG_ONLY),
            ...
            print(f"  done: {window_name:<14}{spacing_mode:<16}{mode_name:<10} ({elapsed:.0f}s elapsed)")

print(f"\n=== RESULTS (n={n_seeds} seeds each) ===\n")
print(f"{'Window':<14}{'Spacing':<16}{'Mode':<10}{'Mean P&L':<12}{'Std':<10}{'MeanLayers':<12}{'MeanTrades':<12}{'DD3%':<8}{'DD4%'}")
for (window_name, spacing_mode, mode_name), r in results.items():
    print(f"{window_name:<14}{spacing_mode:<16}{mode_name:<10}${r['mean_pnl']:<11.2f}${r['std_pnl']:<9.2f}"
          f"{r['mean_max_layers']:<12.1f}{r['mean_trades']:<12.0f}{r['dd3_rate']:<8.1f}{r['dd4_rate']:.1f}")
