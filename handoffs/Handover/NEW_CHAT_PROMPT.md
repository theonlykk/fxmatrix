...
# NEW CHAT PROMPT -- FXMATRIX, 2026-09-25 ~00:30Z
...
(section 9 is the newest); the three newest sections of
...
| | |
|---|---|
| fxmatrix `main` | `bf66d0a` or a docs-only descendant. EA code == tested `238bb66`: `git diff --stat 238bb66 origin/main -- ea/ scripts/ tools/` must be empty |
...
| Suite | 1830/1830 at `238bb66` (the deployed EAs are 1766/1766 at `6c57830`) |
...
| ADR-162 virtual lattice DRAFT + Gemini rulings | `docs/architecture/ADR-162-virtual-lattice.md` |
...
3. **ADR-162 build** in two phases, each tests first, Gemini, Cursor,
   both suite runs, DeepSeek (it moves orders): A = effective-entry
   plumbing + `GRIND_VL_` GV, INERT (no roll ever fires; every existing
   test unchanged); B = trigger, roll, M1 catch-up, WARN, "roll here"
   command. Deploy at a clean boundary (cycle 4 or a dedicated fleet).
