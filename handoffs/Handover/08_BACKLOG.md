...
| B2 | **DECIDED 2026-09-22: carry ON for cycle 3.** Carry shifts EVERY exit, ejected and purgatory-held included; an exit that ignores accrued swap locks in a loss. The accrual-before-modify halt path was fixed first (ADR-155 part A). Set `InpEnableCarryPass=true` in the nine cycle-3 presets before the start | decided |
