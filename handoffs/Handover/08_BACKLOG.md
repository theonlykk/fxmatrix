...
| B2 | **DECIDED 2026-09-22: carry ON for cycle 3.** Carry shifts EVERY exit, ejected and purgatory-held included; an exit that ignores accrued swap locks in a loss. The accrual-before-modify halt path was fixed first (ADR-155 part A). **BLOCKED until C25:** `OnInit` refuses carry today (ADR-151 phase A guard, `fxgrind.mq5:135-138`); only after C25 set `InpEnableCarryPass=true` in the nine cycle-3 presets | decided |
...
| C25 | **ADR-151 Phase B closure -- unblock carry.** `OnInit` returns `INIT_FAILED` if `InpEnableCarryPass` is true (`fxgrind.mq5:135-138`, ADR-151 phase A). Phase B was deferred "before carry is ever enabled": (1) carry-aware target from the swap ledger -- appears done (`a0b4d84`, `1939c8b`, ADR-155A step 11); (2) reconstruction of carry-shifted held targets -- appears done (rebuild uses `Grind_ExitQFormulaTarget`); (3) target promotion with a rank deadband -- unbuilt, possibly moot under the barbell (ranks by entry). Verify (1)-(2) with tests, rule on (3), remove the guard. **Gates cycle 3** (B2) | not started |
