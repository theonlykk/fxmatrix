...
# CURSOR TASK -- Run the ADR-162 Phase B1 DeepSeek R1 audit
...
## CONTEXT
  - `D:\fxmatrix` on branch `adr162-phase-b1`. The inputs are ALREADY
    committed on it (the commit after `3188f72`; suite 2002/2002 on GBPUSD
    and EURUSD at `3188f72`):
      `prompts\deepseek_adr162b1_audit.md` (the brief, 139 lines)
      `prompts\deepseek_adr162b1_audit_config.json` (the runner config)
      `prompts\cursor_adr162_phase_b1.md` (the B1 spec, attached as a doc)
      `prompts\cursor_run_deepseek_adr162b1.md` (this task)
    The runner reads `ea\` from this working copy, so it MUST stay on this
    branch head. The EA code at the head is identical to `3188f72`.
...
1. `git fetch origin`; `git branch --show-current` must print
   `adr162-phase-b1`; `git rev-parse --short HEAD` must equal
   `git rev-parse --short origin/adr162-phase-b1`; report it.
2. `git diff --stat 3188f72 HEAD -- ea/ scripts/` must print nothing.
3. `git status -s` must show no modified tracked files (untracked are
   fine; report them).
4. Brief checks on `prompts\deepseek_adr162b1_audit.md`:
     - `(Get-Content <f>).Count` is 139
     - line 1 begins `This message has a line count at the bottom`
     - line 79 is exactly `## THREATS -- verdict each: HOLDS / BREAKS / NEEDS-FIX`
     - line 139 is exactly `Line count: 139`
5. `D:\candlelab\venv\Scripts\python.exe -m json.tool prompts\deepseek_adr162b1_audit_config.json`
   must succeed (print only "valid" or the error).

## STEP 1 -- Dry run
    D:\candlelab\venv\Scripts\python.exe tools\r1_audit\r1_audit.py --config prompts\deepseek_adr162b1_audit_config.json --dry-run
Expect `Preflight check passed: 9 files verified` (7 code, 1 doc, the
brief) and roughly 285,000 chars. STOP and report if preflight fails or
the size is under 270,000 or over 300,000.

## STEP 2 -- Real run
    D:\candlelab\venv\Scripts\python.exe tools\r1_audit\r1_audit.py --config prompts\deepseek_adr162b1_audit_config.json
...
## STEP 3 -- Mechanical checks on `prompts\deepseek_adr162b1_audit_response.md`
  - total line count and character count
  - YES/NO for each literal: `GIVENS CHECK`, `T-1`, `T-2`, `T-3`, `T-4`,
    `T-5`, `T-6`, `T-7`, `T-8`, `T-9`, `T-10`, `PREMISE VERDICT`,
    `TEST GAPS`
...
## STEP 4 -- Commit the response (on adr162-phase-b1)
    git add prompts/deepseek_adr162b1_audit_response.md
    git commit -m "Add DeepSeek R1 response: ADR-162 Phase B1 audit"
