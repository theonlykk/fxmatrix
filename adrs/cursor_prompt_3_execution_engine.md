...
void HandleEntryFill(ulong order_ticket, double deal_volume,
    ...
    if (layer_idx == -1) {
        ...
        L.instrument                 = g_strongest == 0 && g_weakest == 1
                                       ? INSTRUMENT_EURGBP
                                       : g_strongest == 0 || g_weakest == 0
                                         ? INSTRUMENT_EURUSD
                                         : INSTRUMENT_GBPUSD;
        L.direction                  = (g_strongest > g_weakest)
                                       ? DIRECTION_SELL : DIRECTION_BUY;
        L.lot_size                   = BaseLotSize;
        L.remaining_volume           = BaseLotSize;
    ...
    g_inventory[layer_idx].remaining_volume -= deal_volume;
    ...
    double filled_so_far = g_inventory[layer_idx].lot_size
                         - g_inventory[layer_idx].remaining_volume;
...
6. Confirm MinFillThreshold check uses cumulative filled volume
   (lot_size - remaining_volume), not per-fill volume.
