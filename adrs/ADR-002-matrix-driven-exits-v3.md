...
**Supersedes:** ADR-002 v2 (2026-06-08) — surgical patch to Cases 1 & 2
...
| Finding | Change |
|---------|--------|
| Cases 1 & 2 historical anchor bug | EG_synthetic (live spot) replaced with EG_history = EU_mid_12bars_ago / GB_mid_12bars_ago. Three-step structure: historical anchor → inversion → live passivity buffer. Now structurally identical to Cases 3–6. |
...
```
// Step 1: Historical anchor — signal layer, mid convention
EG_history   = EU_mid_12bars_ago / GB_mid_12bars_ago

// Step 2: Inversion from historical anchor
EG_target    = EG_history × exp(-T)

// Step 3: Passivity buffer — execution layer, live bid/ask
...
6. **ADR-002 v3 (this document)** — Accepted ✅ Gemini approved 2026-06-08
7. ADR-003 — Accepted ✅ Gemini approved 2026-06-08
8. DeepSeek combined audit (ADR-002 v3 + ADR-003) — NEXT
9. MQL5 EA implementation (Cursor) — blocked until combined audit complete
10. MT5 strategy tester validation on historical tick data
11. Practice account deployment (minimum lot size)
12. 4-week observation before any parameter changes
13. Live account deployment after practice validation
