...
//
// remaining_volume is decremented ONLY by OnTradeTransaction.
...
double   g_NudgeThreshold       = 0.0;   // computed at OnInit from NudgePips

//--- Inventory
Layer    g_inventory[];                  // dynamic array of open layers
```

### OnInit skeleton (globals initialisation only)
...
1. Re-read every field in the Layer struct against ADR-002 v4
   ...
   (21 original + r_EU_at_entry, r_GB_at_entry,
   strongest_at_entry, weakest_at_entry).
...
5. Confirm OnInit() initialises g_NudgeThreshold from NudgePips
